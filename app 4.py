...
def hero():
    if os.path.exists(ASSETS_BANNER):
        st.image(ASSETS_BANNER, use_column_width=True)
    st.markdown(f"""
    <div class="hero">
      <h1>{APP_TITLE}</h1>
...
def _bcrypt_hash(pw: str) -> str:
    if not HAS_BCRYPT: raise RuntimeError("bcrypt is required for password hashing")
    return bcrypt.hashpw(pw.encode(), bcrypt.gensalt(rounds=12)).decode()

def _bcrypt_check(pw: str, hashed: str) -> bool:
    if not HAS_BCRYPT: return False
...
def register_user(username: str, password: str) -> Tuple[bool, str]:
    ...
    try:
        users[u] = {"pw_hash": _bcrypt_hash(password), "created_at": datetime.utcnow().isoformat() + "Z", "role": "user"}
        _save_users(users)
